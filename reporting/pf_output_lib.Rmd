...
pf_output(process_output = get_results('pf_ss_exp_nt') %>% mutate(site = site_summ),
...
pf_output(process_output = get_results('pf_ss_anom_nt') %>% mutate(site = site_summ),
...
pf_output(process_output = get_results('pf_ms_nt'),
...
pf_output(process_output = get_results('pf_ss_at') %>% mutate(site = site_summ),
...
pf_output(process_output = get_results('pf_ms_at'),
...
This check displays the area under the curve (AUC) for ONE domain & visit type pair across time, for every site in the cohort. The AUC computation itself takes place in the initial processing function to reduce load on the summarization end. During this computation, the all site mean is used as the "gold standard" against which the site-specific proportion associated with a given domain is compared. 

Two graphs are output:

  * a line graph displaying the proportion of facts per patient/domain/visit type at each site over time, with the AUC values available in the tooltip when hovering over a given line
