...
op <- scv_output(process_output = get_results('scv_ss_nt'),
...
scv_output(process_output = get_results('scv_ss_nt'),
...
scv_output(process_output = get_results('scv_ms_nt'),
...
op <- scv_output(process_output = get_results('scv_ss_at'),
...
scv_output(process_output = get_results('scv_ss_at'),
...
op <- scv_output(process_output = get_results('scv_ms_at'),
...
This check displays the area under the curve (AUC) for ONE concept pair (primary concept provided by the user in the processing function and its mapped pair) across time, for every site in the cohort. The AUC computation itself takes place in the initial processing function to reduce load on the summarization end. During this computation, the all site mean is used as the "gold standard" against which the site-specific proportion associated with a given concept pair is compared. 

Two graphs are output:

  * a line graph displaying the proportion of the primary concept at each site over time, with the AUC values available in the tooltip when hovering over a given line
    + the proportion shown is that of the representation of the mapped concept of interest within the primary concept of interest. for example, if the user provides a SNOMED condition code (code_type = 'cdm'), the y-axis will display the proportion of rows with that SNOMED code that are associated with the source ICD code selected by the user.
  * a bar graph comparing the AUC values for that concept pair at each site against the all site overall AUC value
