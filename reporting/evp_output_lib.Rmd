...
This check displays the area under the curve (AUC) for ONE variable across time, for every site in the cohort. The AUC computation itself takes place in the initial processing function to reduce load on the summarization end. During this computation, the all site mean is used as the "gold standard" against which the site-specific proportion associated with a given variable is compared. 

Two graphs are output:

  * a line graph displaying the proportion of that variable at each site over time, with the AUC values available in the tooltip when hovering over a given line
