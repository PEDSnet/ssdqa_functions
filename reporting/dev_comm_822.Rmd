...
ss_exp_nt <- create_list_input_sepsites(data_tbl=results_tbl('ss_exp_nt') %>% filter(visit_type == 'outpatient') 
...
ggplot(results_tbl('ss_anom_nt') %>% site_anon() %>% collect_new(), 
       aes(x = prop_outlier_site_fact, y = site)) +
  geom_col() +
  facet_wrap(~var_name) +
...
ss_exp_glom <- 
  compute_plot_fot_exp(fot_tbl=results_tbl('ss_exp_at') %>% site_anon() %>% collect_new,
...
```{r ss_anom_at, echo=FALSE}
 ...
 r <- plot_anomaly_diagnostics(.data=results_tbl('ss_anom_at') %>% site_anon() %>% collect_new() %>%
                                 filter(visit_type=='outpatient',domain=='labs', site == 'chop',
                               ...
                               .legend_show = FALSE,
                               .max_anomalies = 0.5, 
                               .title = 'Median Labs Across Time for Outpatient Visits: Anomaly Detection Across Sites',
                               .facet_ncol = 3, 
                               .facet_scales = 'free_x', 
                               .facet_collapse = TRUE, 
                               .interactive = FALSE)
...
set.seed(123)
ms_anom <- kmeans(kmeans_prep, centers=3, nstart=40)
