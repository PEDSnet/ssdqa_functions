...
sites <- c('site 1', 'site 2', 'site 3', 'site 4', 'site 5')

site_palette_base <- colorRampPalette(brewer.pal(8, "Set2"))

site_colors <- setNames(site_palette_base(length(sites)), sites)
...
grid <- gridExtra::grid.arrange(output[[1]], output[[2]], output[[3]], nrow = 2, 
                                layout_matrix = rbind(c(1,2),(3)), 
                                top = 'Median Facts per Patient: \nSingle Site Exploratory')
...
ggsave(
  ...
  width = 35,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_exp_nt.png')
)
...
results_tbl('ss_anom_nt') %>% site_anon() %>% collect_new() %>% 
  ...
  distinct() %>%
  left_join(site_report_config) %>%
ggplot(aes(x = prop_outlier_site_fact, y = site, fill = site)) +
  geom_col() +
  facet_wrap(~var_name) +
  scale_fill_manual(values = site_colors) +
  labs(title = 'Single Site Anomaly Detection: \nProportion of Patients +/- 2 SD away from Mean') +
  theme_classic()
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_anom_nt.png')
)
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_exp_at.png')
)
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_anom_at.png')
)
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_exp_nt.png')
)
...
fviz_cluster(ms_anom, data=kmeans_prep, main = 'K-Means Cluster Analysis: \nMulti-Site Anomaly Detection')
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_anom_nt.png')
)
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_exp_at.png')
)
...
multisite_anom_graph <- produce_multisite_anom(multisite_anomaly_tbl = output_anom,
                                               color_config=color_config_file,
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_anom_at.png')
)
...
age_cohort <- results_tbl('ss_age') %>% 
  site_anon() %>% collect_new() %>%
  mutate(n_w_fact = format(n_w_fact, big.mark = ',', scientific = FALSE),
         site = paste0(site, '\n(N = ', n_w_fact, ')')) %>%
  filter(age_grp != 'None')

ss_age <- create_list_input_facet_sepvarname(data_tbl= age_cohort,
                                             visit_type_nm='all',
...
grid <- gridExtra::grid.arrange(output[[1]], output[[2]], output[[3]], nrow = 2, 
                                layout_matrix = rbind(c(1,2),(3)),
                                top = 'Single Site Age Group Stratification')
...
ggsave(
  ...
  width = 35,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_age.png')
)
...
point <- results_tbl('ms_age') %>% site_anon() %>% filter(visit_type == 'all' & age_grp != 'None') %>% 
  collect_new() %>%
  mutate(n_w_fact = format(n_w_fact, big.mark = ',', scientific = FALSE),
         site_lab = paste0(site, ' (N = ', n_w_fact, ')')) %>%
ggplot(aes(x=var_name,y=median_site_without0s, colour=site))+
  geom_point_interactive(aes(data_id=site_lab, tooltip = site_lab), size=3)+
  geom_point(aes(x=var_name, y=median_all_without0s), shape=8, size=3, color="black") +
  #geom_point(aes(x=var_name, y=median_all_without0s), shape=8, size=3, color="black")+
  facet_wrap(~age_grp, scales="free_x", ncol=2)+
  theme_bw()+
  labs(title = 'Multi-Site Age Group Stratification') +
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_age.png')
)
...
cancer_cohort <- results_tbl('ss_cancer') %>% 
  site_anon() %>% collect_new() %>%
  mutate(n_w_fact = format(n_w_fact, big.mark = ',', scientific = FALSE),
         site = paste0(site, '\n(N = ', n_w_fact, ')'))

ss_cancer <- create_list_input_facet_sepvarname(data_tbl= cancer_cohort,
                                             visit_type_nm='all',
                                             facet_var_nm='flag',
                                             var_name_list=var_name_list)
...
grid <- gridExtra::grid.arrange(output[[1]], output[[2]], output[[3]], nrow = 2, 
                                layout_matrix = rbind(c(1,2),(3)),
                                top = 'Single Site Codeset Stratification: Cancer Diagnosis')
...
ggsave(
  ...
  width = 35,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_cancer_bar.png')
)


pyramid_chart(data = results_tbl('ss_cancer') %>% site_anon() %>% 
                filter(site == 'site 3' & visit_type == 'all') %>% collect(),
              x = var_name,
              y = median_site_without0s,
              group = flag,
              sort = 'ascending',
              title='site 3, all visits: median facts by presence of cancer dx',
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_cancer_pyr.png')
)
...
cardiac_cohort <- results_tbl('ss_cardiac') %>% 
  site_anon() %>% collect_new() %>%
  mutate(n_w_fact = format(n_w_fact, big.mark = ',', scientific = FALSE),
         site = paste0(site, '\n(N = ', n_w_fact, ')'))

ss_cardiac <- create_list_input_facet_sepvarname(data_tbl= cardiac_cohort,
                                             visit_type_nm='all',
                                             facet_var_nm='flag',
                                             var_name_list=var_name_list)
...
grid <- gridExtra::grid.arrange(output[[1]], output[[2]], output[[3]], nrow = 2, 
                                layout_matrix = rbind(c(1,2),(3)),
                                top = 'Single Site Codeset Stratification: Cardiac Specialty Visit')
...
ggsave(
  ...
  width = 35,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_cardiac_bar.png')
)

pyramid_chart(data = results_tbl('ss_cardiac') %>% site_anon() %>% 
                filter(site == 'site 3' & visit_type == 'all') %>% collect(),
              x = var_name,
              y = median_site_without0s,
              group = flag,
              sort = 'ascending',
              title='site 3, all visits: median facts by presence of cardiac specialty visit',
              bar_colors = c('#6fa8dc','#93c47d'))
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ss_cardiac_pyr.png')
)
...
point <- results_tbl('ms_cancer') %>% site_anon() %>% filter(visit_type == 'all') %>% collect_new() %>%
  mutate(n_w_fact = format(n_w_fact, big.mark = ',', scientific = FALSE),
         site_lab = paste0(site, ' (N = ', n_w_fact, ')')) %>%
ggplot(aes(x=var_name,y=median_site_without0s, colour=site))+
  geom_point_interactive(aes(data_id=site_lab, tooltip = site_lab), size=3)+
  ...
  theme_bw()+
  labs(title = 'Multi-Site, All Visits: Cancer Diagnosis') +
#  theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1))+
  coord_flip()
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_cancer.png')
)

point2 <- results_tbl('ms_cardiac') %>% site_anon() %>% filter(visit_type == 'all') %>% collect_new() %>%
  mutate(n_w_fact = format(n_w_fact, big.mark = ',', scientific = FALSE),
         site_lab = paste0(site, ' (N = ', n_w_fact, ')')) %>%
ggplot(aes(x=var_name,y=median_site_without0s, colour=site))+
  geom_point_interactive(aes(data_id=site_lab, tooltip = site_lab), size=3)+
  ...
  theme_bw()+
  labs(title = 'Multi-Site, All Visits: Cardiac Specialty Visits') +
#  theme(axis.text.x=element_text(angle=90, vjust=0.5, hjust=1))+
  coord_flip()
...
ggsave(
  ...
  width = 20,
  units = c('cm'),
  filename = paste0(config('base_dir'),'/results/ms_cardiac.png')
)
