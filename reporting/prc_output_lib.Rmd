...
This bar graph displays the proportion of patients that fall into each event category. The tooltip contains the name of the event, the proportion for the category, and the average number of times events A and B occur within each patient record. The computation of the mean includes 0s to ensure that patients with no event are still taken into account.
...
This heatmap displays the Jaccard similarity index for categories of event occurrence counts. Here we want to see when different numbers of each event occur together within the same patient. For example, you can see that 0 Event B occurrences and 1 Event A occurrence occur together frequently.

The bins are decided on procedurally (**subject to change**). Counts less than 10 are given their own bin (i.e. 0, 1, 2, etc), counts 10-100 are binned by tens (i.e. 10-20, 20-30), counts 100-1000 are binned by hundreds (i.e. 100-200, 200-300), and counts greater than 1000 are binned by thousands (i.e. 1000-2000). This was done to try to preserve granularity where needed but still aim to consolidate and reduce computation time.

The top 20 most frequently occurring bins are chosen by the function to save space on the graph. The higher the Jaccard index, the more the two bins occur together.
...
This uses the same Jaccard similarity index computed for Single Site, Anomaly, No Time and runs it through the rule & hotspots based anomaly detection function to find anomalous sites for each pair of event counts.
