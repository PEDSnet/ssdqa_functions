...
compute_evp_pcnt <- function(cohort,
  ...
  for(i in 1:length(evp_list)){
    ...
    fact_pts <- domain_tbl %>%
      ...
      summarise(concept_pt_ct = n_distinct(patid),
...
compute_evp_ssanom_pcnt <- function(cohort,
  ...
  jaccards <- compute_jaccard_evp_pcnt(jaccard_input_tbl = facet_list[[i]]) %>%
    mutate(grp = grp)
...
compute_jaccard_evp_pcnt <- function(jaccard_input_tbl) {
  ...
  persons_concepts_cts <- 
    persons_concepts %>% 
    group_by(variable) %>% 
    summarise(concept_person_ct=n_distinct(patid))
